const countries = [
 ...
 function populateDropdown() {
    ...
    // Update flag image when a country is selected
    select.addEventListener("change", function() {
        const selectedCountry = countries.find(country => country.code === this.value);
        if (selectedCountry) {
            flagImg.src = `https://flagicons.lipis.dev/${selectedCountry.flag_4x3}`;
        } else {
            flagImg.src = ""; // Clear the image if no country is selected
        }
    });
}
